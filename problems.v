...
Proof.
unfold Base, L1 in |- *.
apply DiLn_qimp_con; auto with v62.
exists (origin (base t)); split; auto with v62.
rewrite (auxs1 t); auto with v62.
Qed.
Hint Resolve lemma1.

Let lemma2 : DiLn Base L3.
Proof.
unfold L3, Base, C in |- *; auto with v62.
Qed.
Hint Resolve lemma2.
...
Proof.
unfold B, L1 in |- *.
apply cong_eqln_apt with (l := ln (reverse (Side1 t))); auto with v62.
Qed.
Hint Resolve lemma5'.

Let lemma2' : DiLn L1 L4.
Proof.
unfold L1, L4 in |- *; auto with v62.
Qed.
Hint Resolve lemma2'.
...
Proof.
unfold L3 at 1 in |- *.
apply cong_par_con with (m := Base); auto with v62.
Qed.
Hint Resolve lemma3.
...
Proof.
unfold L4 at 1 in |- *.
apply cong_par_con with (m := L1); auto with v62.
Qed.
Hint Resolve lemma4.
...
lapply (constructive_uniqueness_for_parallels Base L3 B);
 [ intro H'2; elim H'2;
    [ intro H'3; elim H'3; [ intro H'4; clear H'3 H'2 | trivial with v62 ]
    | intro H'3; clear H'2 ]
 | idtac ]; auto with v62.
unfold B, Base in H'4.
elim (inc_ln2 (base t)); auto with v62.
unfold L3 in H'3.
elim (Ax1_i Base C); auto with v62.
Qed.
Hint Resolve lemma5.
...
lapply (constructive_uniqueness_for_parallels L1 L4 C);
 [ intro H'4; elim H'4;
    [ intro H'5; elim H'5; [ intro H'6; clear H'5 H'4 | trivial with v62 ]
    | intro H'5; clear H'4 ]
 | idtac ]; auto with v62.
unfold C, L1 in H'6.
generalize H'6; rewrite (auxs3 t); intro H'.
elim (inc_ln1 (Side1 t)); auto with v62.
elim (Ax1_i L1 B); auto with v62.
Qed.
Hint Resolve lemma6.

Let lemma7 : DiPt C D.
Proof.
unfold D in |- *; auto with v62.
Qed.
...
Proof.
unfold L1 at 1 in |- *; auto with v62.
Qed.
Hint Resolve lemma9.
...
unfold S2, L1, L3 in |- *; simpl in |- *; auto with v62.
...
Proof.
apply Uniqueness_of_constructed_lines.
unfold S4, L4 in |- *; simpl in |- *; auto with v62.
...
simpl in |- *.
rewrite <- (auxs3 t).
unfold C in |- *; auto with v62.
unfold S1, S2, S4, B in |- *; simpl in |- *; auto with v62.
apply cong_eqln_spar with (m := L3); auto with v62.
unfold L3, S1, Base, C in |- *; auto with v62.
apply cong_eqln_spar with (m := L4); auto with v62.
unfold L4 in |- *.
apply sym_SPar.
apply cong_eqln_spar with (m := L1); auto with v62.
Qed.
