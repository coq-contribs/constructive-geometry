...
lapply (el_ax (base t) (ln (base t)) (ln (Side2 t))); trivial with v62.
intro H.
...
rewrite (auxs2 t).
intro H'2.
elim H'2; [ trivial with v62 | intro H'0; clear H'2 ].
elim (inc_ln2 (Side2 t)); trivial with v62.
Qed.
...
generalize (inc_ln2 (base t)); generalize (inc_ln1 (base t)).
unfold Incident in |- *; tauto.
elim H'0; [ intro H'1; clear H'0 | trivial with v62 ].
generalize (inc_ln1 (reverse (Side1 t))).
rewrite <- (ext_rev (Side1 t)).
rewrite <- (auxs1 t).
intro H'3; elim H'3; auto with v62.
apply cong_eqln_diln with (m := ln (Side1 t)); auto with v62.
Qed.
...
apply cong_eqln_apt with (l := ln base); auto with v62.
Qed.
...
Proof.
split; apply cong_eqpt_inc with (a := pt x); auto with v62.
Qed.
...
Proof.
intuition.
apply Uniqueness_of_constructed_points; trivial with v62.
Qed.
...
lapply (thm4_6 (reverse (Seg b c H2)) (Seg a b H1));
 [ intro H'1; elim H'1; trivial with v62 | idtac ].
generalize (inc_ln2 (Seg a b H1)); auto with v62.
Qed.
...
lapply (thm4_6 (Seg a c H2) (Seg a b H1));
 [ intro H'1; elim H'1; trivial with v62 | idtac ].
generalize (inc_ln1 (Seg a b H1)); auto with v62.
Qed.
...
Proof.
intros x c H'.
apply cong_eqln_inc with (l := ln x); auto with v62.
Qed.
...
lapply (cmp_apt_diln (pt x) c (line1 x));
 [ intro H'3; elim H'3; [ trivial with v62 | intro H'4; clear H'3 ] | idtac ];
 trivial with v62.
elim (inc_pt1 x); trivial with v62.
Qed.
...
lapply (cmp_apt_diln (pt x) c (line2 x));
 [ intro H'3; elim H'3; [ trivial with v62 | intro H'4; clear H'3 ] | idtac ];
 trivial with v62.
elim (inc_pt2 x); trivial with v62.
Qed.
...
lapply
 (el_ax (Seg origin extremity Seg_cond) (ln (Seg origin extremity Seg_cond))
    (line1 x)); [ intro H'7 | trivial with v62 ].
simpl in H'7.
elim H'7; clear H'7; (intro H'3; elim H'3; (clear H'3; intro H'4)).
elim (inc_ln1 (Seg origin extremity Seg_cond)); auto with v62.
elim (inc_ln2 (Seg origin extremity Seg_cond)); auto with v62.
...
apply cong_eqpt_inc with (a := b); trivial with v62.
Qed.
...
apply cong_eqln_inc with (l := m); trivial with v62.
Qed.
