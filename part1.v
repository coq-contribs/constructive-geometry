...
lapply (el_ax x l (ln x)); trivial with v62.
tauto.
Qed.
...
Proof.
intros l m H'.
lapply (cmp_con_diln l m l); trivial with v62.
intro H'0; elim H'0; auto with v62.
intro H'1; elim apart_con.
intro H'2; elim (H'2 l); trivial with v62.
Qed.
...
Proof.
intros a b l H' H'0.
elim (cmp_apt_dipt a b l); auto with v62.
intro H'1; elim H'0; trivial with v62.
Qed.
...
Proof.
intros a l m H' H'0.
elim (cmp_apt_diln a l m); auto with v62.
intro H'1; elim H'0; trivial with v62.
Qed.
...
apply cong_eqpt_apt with (a := b); auto with v62.
Qed.
...
apply cong_eqln_apt with (l := m); auto with v62.
Qed.
...
Proof.
intros l m n H' H'0.
elim (cmp_con_diln l m n); auto with v62.
intro H'1; elim H'0; trivial with v62.
Qed.
...
apply cong_eqln_con with (m := n); auto with v62.
Qed.
...
intros H'1 H'2; elim (H'2 a b c); trivial with v62.
intro H'3; elim H'0; trivial with v62.
Qed.
...
unfold Separating at 1 in |- *.
intros H'1 H'2; elim (H'2 l m n); trivial with v62.
intro H'3; elim H'0; trivial with v62.
Qed.

Theorem eqln_imp_par : forall l m : Line, EqLn l m -> Par l m.
Proof.
unfold Par, EqLn, Negation in |- *; red in |- *; auto with v62.
Qed.
...
intro H'; split.
apply cong_eqln_par with (m := m); trivial with v62.
apply cong_eqln_diln with (m := m); trivial with v62.
Qed.
...
intro H'; elim H'.
intros a b H'0.
apply (Seg b a); auto with v62.
Defined.

Theorem orig_rev : forall x : Segment, origin x = extremity (reverse x).
Proof.
intro x; elim x; simpl in |- *; auto with v62.
Qed.
...
rewrite <- (ext_rev x); auto with v62.
rewrite <- (orig_rev x); auto with v62.
Qed.
...
rewrite <- (line2_flip x); auto with v62.
rewrite <- (line1_flip x); auto with v62.
Qed.
...
Proof.
cut (Equivalence Line EqLn); auto with v62.
intro H'; elim H'.
...
red in H'2.
apply H'2 with (y := ln y); auto with v62.
Qed.
